...
const checkCaseIds = async (caseNumbers = []) => {
  const sql = `
    SELECT cd.case_number, u.mobile_number, cd.diary_number
    FROM subscribed_cases sc
    LEFT JOIN case_details cd ON sc.case_id = cd.id
    JOIN users u ON sc.user_id = u.id
    WHERE cd.case_number = ANY($1)
      AND cd.court = 'District Court'
  `;

  const { rows } = await db.query(sql, [caseNumbers]);
  return rows;
};
