...
const fetchSupremeCourtOTF = async (caseType, caseNumber, caseYear, diaryNumber) => {
  ...
  try {
    ...
    if (diaryNumber !== "" && diaryNumber !== null && diaryNumber !== undefined) {
      await page.goto(`https://www.sci.gov.in/daily-order-diary-no/`);
    } else {
      await page.goto('https://www.sci.gov.in/daily-order-case-no/');
    }

    await wait(2000);

    if (diaryNumber !== "" && diaryNumber !== null && diaryNumber !== undefined) {
      await enterDiaryNumber(page, diaryNumber);
    } else {
      await setCaseTypeAndSetValue(page, caseType, caseNumber, caseYear);
    }
