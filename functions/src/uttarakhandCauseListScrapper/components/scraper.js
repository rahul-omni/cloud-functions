...
async function getCasesFromPDF(page, link) {
  try {
    ...
    const regex =
      /\b([A-Z]{2,15})\s*([\/-])\s*(\d{1,8})\s*\2\s*(\d{4})\b/g;
    ...
    while ((match = regex.exec(text)) !== null) {
      found.add(
        `${match[1]}${match[2]}${match[3]}${match[2]}${match[4]}`
      );
    }

    const cases = [...found];

    console.log("[getCasesFromPDF] Total unique cases:", cases.length);
