...
const { setDateRange } = require('./components/utils');

// Get OpenAI key lazily (not at module load time)
const getOpenAIKey = () => {
  try {
    ...
    throw new Error('🔴  OPENAI_API_KEY missing or not configured');
  }
};

/* ─── arithmetic captcha via OpenAI Vision ─── */
const solveCaptcha = async (buf) => {
  const KEY = getOpenAIKey(); // Get key when function is called, not at module load
...
/* ─── main routine with date range ─── */
const fetchSupremeCourtJudgmentsByDateRange = async (fromDate, toDate) => {
  console.log(`[start] [fetchSupremeCourtJudgmentsByDateRangeNotify] Scraping judgments for date range: ${fromDate} to ${toDate}`);
  ...
  try {
    ...
    // Fill the date range form using the helper function
    ...
    console.log('[info] [fetchSupremeCourtJudgmentsByDateRangeNotify] Captcha solved:', answer);
    await page.type('#siwp_captcha_value_0', answer);
    await wait(600);

    // Submit and wait for results
    await page.click('input[value="Search"]');
    console.log('[info] [fetchSupremeCourtJudgmentsByDateRangeNotify] Submitted and waiting for results');
    await wait(12000);

    console.log('[info] [fetchSupremeCourtJudgmentsByDateRangeNotify] Results received');
    ...
    try {
      ...
      console.error('[error] [fetchSupremeCourtJudgmentsByDateRangeNotify] Failed to get results:', error.message);
      throw error;
    } 

  } catch (error) {
    console.error('[error] [fetchSupremeCourtJudgmentsByDateRangeNotify] Error during scraping service: ', error);
    throw error;
  } finally {
    await browser.close();
    console.log("[end] [fetchSupremeCourtJudgmentsByDateRangeNotify] Supreme Court Scraping completed successfully");
  }
}

/* ─── main routine (legacy - single date) ─── */
...
module.exports = {
  ...
  fetchSupremeCourtJudgmentsByDateRange
}