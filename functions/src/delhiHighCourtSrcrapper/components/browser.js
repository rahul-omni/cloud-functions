...
async function navigateToMainPage(page, modalHandled) {
  ...
  // Allow site JS to settle
  await wait(3000);

  // Case Wise tab is default on /app
  // Clicking it again causes a reload + easyTicker error
  console.log('[navigateToMainPage] Case Wise tab is default — skipping click');

  // Ensure Case Wise form is present before continuing
  await page.waitForSelector('#case_type', { visible: true });

  console.log('[navigateToMainPage] Main page ready');
}
...
async function setDiaryNumberFields(page, diaryNumber, caseTypeValue) {
  ...
  await page.waitForSelector('#case_number', { visible: true });

  await page.click('#case_number', { clickCount: 3 });
  await page.type('#case_number', caseNumber);
  await wait(1000);
